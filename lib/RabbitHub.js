 ...
 */
RabbitHub.prototype.initExchange = function(callback) {
  this.exchange = this.connection.exchange('renren-exchange', { type: 'direct' }, callback);
}
 ...
 * @return {object|null}
 */
RabbitHub.prototype.createQueue = function(name, callback) {
  if (this.queue[name] !== undefined) {
    Logger('error', 'RabbitMQ already has a queue with name ' + name, 'rabbitmq');
    return null;
  }

  var queue = this.connection.queue(name, callback);
